...
class LocalExecutionSpec extends UnitSpec {
  ...
  "Multiple topologies each with multiple stages" should "when run in parallel process events" in {
    // start the cluster manager
    val cmActor = ClusterManager.start()
    // final destination sink
    val destinationStore = buildDummyDestinationStream("final_destination")
   ...
   "Multiple topologies each with multiple stages and partitions" should "process events" in {
    // start the cluster manager
    val cmActor = ClusterManager.start()
